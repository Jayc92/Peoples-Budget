...
// ── LIVE EVENTS are loaded from Supabase at runtime (see getActiveEvent). ──
...
// ── STORAGE & DATA: see ./lib/api (Supabase + localStorage) ──────────────────
...
export default function App() {
  ...
  const [pulse, setPulse] = useState(null);          // averaged node for current results filter
  ...
  // boot: restore local profile + load the active event from Supabase
  useEffect(() => {
    (async () => {
      const p = loadProfile();
      if (p) { setSavedProfile(p); if (p.respondedEvents) setRespondedEvents(p.respondedEvents); }
      try { setActiveEvent(await getActiveEvent()); } catch (e) { console.error(e); }
      setBooting(false);
    })();
  }, []);
  ...
  const persist = (extra = {}) => { saveProfile({ username: form.username, incomeIdx: form.incomeIdx, useExact: form.useExact, exactIncome: form.exactIncome, filing: form.filing, state: form.state, county: form.county, payFreq: form.payFreq, alloc, respondedEvents, ...extra }); };
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (e.code === "rate_limited") setSubmitError("You've already submitted recently — your earlier budget still counts. You can update again in a few hours.");
      else setSubmitError("Couldn't reach the server, but your budget is saved on this device.");
    }
    setSaving(false);
    setScreen("results");
  };
  const handleEventResponse = async (choice) => {
    ...
    saveProfile({ ...(savedProfile || {}), username: form.username || savedProfile?.username, respondedEvents: updated });
    try {
  ...
  // Load community averages when viewing results in "public" mode (or filter changes)
  useEffect(() => {
    if (screen !== "results" || compareMode !== "community") return;
    setPulseLoading(true);
    const args = filter.type === "all" ? { dim: "all" }
      : filter.type === "region" ? { dim: "region", region: user?.state }
      : { dim: "bracket", bracket: filter.value };
    getPulse(args).then(p => { setPulse(p); setPulseLoading(false); }).catch(() => { setPulse(null); setPulseLoading(false); });
  }, [screen, compareMode, filter]);

  // Load the event tally when opening the event screen
  useEffect(() => {
    if (screen === "event" && activeEvent) {
      getEventTally(activeEvent.id).then(setEventTally).catch(() => {});
    }
  }, [screen, activeEvent]);

  const S = {
    app: { minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'DM Sans', sans-serif", position: "relative" },
        ...
        {activeEvent && (
          <div onClick={() => setScreen("event")} style={{ ...S.card, padding: 18, marginBottom: 20, cursor: "pointer", display: "flex", alignItems: "center", gap: 14, background: "rgba(224,92,92,0.08)", border: "1px solid rgba(224,92,92,0.3)" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600 }}>{activeEvent.title}</div>
            </div>
    ...
    return (
      <div style={S.app}>{S.fonts}<div style={S.grain} /><div style={S.accent} />
        <div style={S.container}>
          ...
          {activeEvent && !respondedEvents[activeEvent.id] && (
            <div onClick={() => setScreen("event")} style={{ ...S.card, padding: 14, marginBottom: 16, cursor: "pointer", display: "flex", alignItems: "center", gap: 12, background: "rgba(224,92,92,0.08)", border: "1px solid rgba(224,92,92,0.3)" }}>
              ...
              <span style={{ fontSize: 13, flex: 1 }}><strong style={{ color: C.red }}>Live:</strong> {activeEvent.title} — tap to weigh in</span>
            </div>
          )}
  ...
  // ── EVENT ──
  if (screen === "event") {
    const tally = eventTally || { increase: 0, same: 0, decrease: 0 };
    const totalResp = tally.increase + tally.same + tally.decrease;
    const myChoice = respondedEvents[activeEvent.id] || eventChoice;
    const focusBucket = TIER_BY_ID[activeEvent.tier].buckets.find(b => b.id === activeEvent.bucketId);
    const opts = [
      ...
      <div style={S.app}>{S.fonts}<div style={S.grain} /><div style={S.accent} /><div style={S.accent2} />
        <div style={S.container}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            <span style={S.pill(C.red)}>{activeEvent.badge}</span>
            <span style={{ fontSize: 11, color: "#666", display: "flex", alignItems: "center", gap: 4 }}><Radio size={12} /> Live event</span>
          </div>
          <h2 style={S.h2}>{activeEvent.title}</h2>
          <p style={{ color: "#999", fontSize: 14, lineHeight: 1.6, marginBottom: 20 }}>{activeEvent.body}</p>
          <div style={{ ...S.card, padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 600 }}>{activeEvent.prompt}</div>
            </div>
            {!myChoice ? (
  ...
  // ── RESULTS ──
  if (screen === "results") {
    const communityCount = pulse?.count || 0;
    return (
      <div style={S.app}>{S.fonts}<div style={S.grain} /><div style={S.accent} />
        <div style={S.container}>
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            <h2 style={S.h2}>Your Budget vs. {compareMode === "gov" ? "Reality" : "The Public"}</h2>
            <p style={{ color: "#777", fontSize: 14 }}>{compareMode === "gov" ? "Where you'd put your money, next to what government actually does." : pulseLoading ? "Loading…" : `Compared against ${communityCount.toLocaleString()} budget${communityCount === 1 ? "" : "s"}.`}</p>
            {submitError && <p style={{ color: "#F0A500", fontSize: 12, marginTop: 8 }}>{submitError}</p>}
          </div>
          ...
          {TIERS.map(t => {
            ...
            const ref = compareMode === "gov" ? Object.fromEntries(t.buckets.map(b => [b.id, b.real])) : (pulse?.[t.id] || {});
  ...
  // ── PULSE ──
  if (screen === "pulse") {
    return <PulseScreen S={S} back={back} setScreen={setScreen} />;
  }
}
...
function PulseScreen({ S, back, setScreen }) {
  const [tierId, setTierId] = useState("federal");
  const [pulse, setPulse] = useState(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    setLoading(true);
    getPulse({ dim: "all" }).then(p => { setPulse(p); setLoading(false); }).catch(() => { setPulse(null); setLoading(false); });
  }, []);
  const tier = TIER_BY_ID[tierId];
  const count = pulse?.count || 0;
  const pubAvg = pulse?.[tierId] || {};
  ...
  return (
    <div style={S.app}>{S.fonts}<div style={S.grain} /><div style={S.accent} /><div style={S.accent2} />
      <div style={S.container}>
        ...
        <p style={{ color: "#777", fontSize: 14, marginBottom: 16 }}>{loading ? "Loading the public's budgets…" : count > 0 ? `Across ${count.toLocaleString()} budget${count === 1 ? "" : "s"}, here's where the public diverges from government — no spin, just the numbers.` : "No budgets in yet — be the first and the public picture starts here."}</p>
        <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
          {TIERS.map(t => (
